...
(function (Chart) {
    var PercentPiePlugin = {
        id: "percentPie",

        beforeInit: function (chartInstance, pluginOptions) {
            if (!pluginOptions.enable) return;

            var format = new Joove.ValueFormat(pluginOptions.format);
            ...
            chartInstance.options.tooltips.callbacks.label = function (tooltipItem, data) {
                ...
                var labelName = data.datasets[datasetIndex].items[index].Label;
                var originalValue = (format != null) ?
                    format.formatNumber(data.originalData[datasetIndex][index]) : data.originalData[datasetIndex][index];
                return (
                    labelName + ": " + label +
                    "% (" +
                    originalValue +
        ...
        beforeUpdate: function (chartInstance, pluginOptions) {
            ...
            var visibles = datasets.map(function (dataset) {
                ...
                for (var i in dataset._meta) {
                    return dataset._meta[i].data.map(function (data) {
                        return data.hidden;
                    });
                }
            });

            var isHidden = function (i, j) {
                if (visibles == null || visibles[i] == null) return false;

                return visibles[i][j] != null && visibles[i][j] == true;
            }

            var totals = datasets.map(function (dataset, i) {
                return dataset.data.reduce(function(x, y, j) {
                    if (isHidden(i, j)) {
                        return x;
                    } 
                    return x + y;
            ...
            datasets.forEach(function(dataset, i) {
                dataset.data = dataset.data.map(function(val, j) {
                    ...
                    return val ? Math.round(val * 1000 / totals[i]) / 10 : 0;
                });
            });
        ...
        _setOriginalData: function (chartInstance) {
            var allData = chartInstance.data.datasets.map(function (dataset) {
                return (dataset.data || []).splice(0);
            });
            chartInstance.data.originalData = allData;
        }
    };
    Chart.pluginService.register(PercentPiePlugin);
}.call(this, Chart));

(function (Chart) {
    var LegendTextOverflowPlugin = {
        id: "legendTextOverflow",

        beforeInit: function (chartInstance) {
        	var pieChartConfig = chartInstance.config.options.plugins.percentPie;

        	if (pieChartConfig == null || pieChartConfig.enable === false) {
        		chartInstance.options.tooltips.callbacks.title = function (tooltipItem, data) {
        			var scales = this._chartInstance.config.options.scales;
					
					var label = tooltipItem[0].xLabel;
                    if (scales != null && scales.xAxes[0].formatNumber != null) {
                        label = scales.xAxes[0].ticks.callback(label, tooltipItem[0].index);
	                }
        		...
        		chartInstance.options.tooltips.callbacks.label = function (tooltipItem, data) {
                    ...
                    if (scales != null && scales.yAxes[0].formatNumber != null) {
                        if (typeof value === "object") {
                            var valueTmp = scales.yAxes[0].ticks.callback(value.y, index);
                            if (valueTmp.indexOf('%') === valueTmp.length - 1) {
                                valueTmp = numeral(value).format('0.00%');
                            }
                            value.y = valueTmp;
                        } else {
                            var valueTmp = scales.yAxes[0].ticks.callback(value, index);
                            if (valueTmp.indexOf('%') === valueTmp.length - 1) {
                                valueTmp = numeral(value).format('0.00%');
                            }
                            value = valueTmp;
        ...
        beforeUpdate: function (chartInstance) {
            var options = chartInstance.options.legendTextOverflow;

            var maxTextWidth = (options != null) ? options.maxLabelWidth || 25 : 25;

            var handleOverflow = function (label, i) {
                if (typeof label === "string") {
                    if (label.length > maxTextWidth) {
                        return label.substr(0, maxTextWidth) + "...";
                    }
                    return label;
                } else {
                    return label;
            ...
            chartInstance.data.datasets = chartInstance.data.datasets.map (function (dataset) {
                dataset.label = handleOverflow(dataset.label);
                return dataset;
            });
            chartInstance.data.labels = chartInstance.data.labels.map(handleOverflow);
        }
    };

    Chart.pluginService.register(LegendTextOverflowPlugin);
}.call(this, Chart));
...
(function () {
    function d() { this.drawDataset = this.drawDataset.bind(this) } "undefined" === typeof Chart ? console.warn("Can not find Chart object.") : (d.prototype.beforeDatasetsUpdate = function (a) { if (this.parseOptions(a) && "outside" === this.position) { var b = 1.5 * this.fontSize + this.outsidePadding; a.chartArea.top += b; a.chartArea.bottom -= b } }, d.prototype.afterDatasetsDraw = function (a) { this.parseOptions(a) && (this.labelBounds = [], a.config.data.datasets.forEach(this.drawDataset)) }, d.prototype.drawDataset = function (a) {
        for (var b =
            this.ctx, q = this.chartInstance, f = a._meta[Object.keys(a._meta)[0]], k = 0, g = 0; g < f.data.length; g++) {
                var h = f.data[g], c = h._view; if (0 !== c.circumference || this.showZero) {
                    switch (this.render) {
                    ...
                    }"function" === typeof this.render && (e = this.render({ label: q.config.data.labels[g], value: a.data[g], percentage: p, dataset: a, index: g }), "object" === typeof e && (e = this.loadImage(e))); if (e) {
                        b.save(); b.beginPath(); b.font = Chart.helpers.fontString(this.fontSize, this.fontStyle, this.fontFamily); if ("outside" === this.position || "border" === this.position) {
                            var l = c.outerRadius / 2; var d, m = this.fontSize + this.textMargin; var n = c.startAngle + (c.endAngle - c.startAngle) /
                                2; "border" === this.position ? d = (c.outerRadius - l) / 2 + l : "outside" === this.position && (d = c.outerRadius - l + l + m); n = { x: c.x + Math.cos(n) * d, y: c.y + Math.sin(n) * d }; if ("outside" === this.position) { n.x = n.x < c.x ? n.x - m : n.x + m; var r = c.outerRadius + m }
                        } else l = c.innerRadius, n = h.tooltipPosition(); m = this.fontColor; "function" === typeof m ? m = m({ label: q.config.data.labels[g], value: a.data[g], percentage: p, text: e, backgroundColor: a.backgroundColor[g], dataset: a, index: g }) : "string" !== typeof m && (m = m[g] || this.options.defaultFontColor); if (this.arc) r ||
    ...
    }, d.prototype.checkTextBound = function (a, b, q, f) {
        for (var k = this.labelBounds, g = 0; g < k.length; ++g) {
            for (var h = k[g], c = [[a, q], [a, f], [b, q], [b, f]], e = 0; e < c.length; ++e) {
                var d = c[e][0],
                l = c[e][1]; if (d >= h.left && d <= h.right && l >= h.top && l <= h.bottom) return !1
            } c = [[h.left, h.top], [h.left, h.bottom], [h.right, h.top], [h.right, h.bottom]]; for (e = 0; e < c.length; ++e)if (d = c[e][0], l = c[e][1], d >= a && d <= b && l >= q && l <= f) return !1
        } k.push({ left: a, right: b, top: q, bottom: f }); return !0
    }, d.prototype.measureText = function (a) { return "object" === typeof a ? { width: a.width, height: a.height } : this.ctx.measureText(a) }, d.prototype.fillText = function (a, b, d) {
        var f = this.ctx; "object" === typeof a ? f.drawImage(a, b.x - a.width / 2, b.y - a.height / 2,
            a.width, a.height) : (f.fillStyle = d, f.textBaseline = "top", f.textAlign = "center", this.textShadow && (f.shadowOffsetX = this.shadowOffsetX, f.shadowOffsetY = this.shadowOffsetY, f.shadowColor = this.shadowColor, f.shadowBlur = this.shadowBlur), f.fillText(a, b.x, b.y - this.fontSize / 2), f.shadowBlur = 0, f.shadowColor = "rgba(0,0,0,0)")
    }, d.prototype.loadImage = function (a) { var b = new Image; b.src = a.src; b.width = a.width; b.height = a.height; return b }, d.prototype.drawArcText = function (a, b, d, f) {
        var k = this.ctx, g = d.x, h = d.y, c = d.startAngle; d =
        ...
        beforeInit: function (a) { a.pieceLabel = new d }, beforeDatasetsUpdate: function (a) { a.pieceLabel.beforeDatasetsUpdate(a) },
    ...
    function downsampleChart(chartInstance) {
        ...
        for (var i = 0; i < datasets.length; i++) {
            ...
            dataToDownsample = dataToDownsample.map(function (value, i) {
				...
				const date = CLMS.Framework.DateTime.ParseExact(chartInstance.data.labels[i], Joove.Core.ApplicationLocale(), dateFormat.GeneralLongTimePattern());
                return {
                    x: (date.isValid() ? date : moment(chartInstance.data.labels[i])).toDate().getTime(),
                    y: value
                }
            });
            
            var data = downsample(dataToDownsample, threshold)
                .linq.orderBy(function (x) { return x.x; }).toArray();
            var dataParsed = [], dataLabels = [];
            for (var i = 0; i < data.length; i++) {
                dataLabels.push(new Date(data[i].x));
                dataParsed.push(data[i].y);
            }
